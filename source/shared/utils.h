...
public:
    ...
    {
        auto Result = std::find_if(Parts.begin(), Parts.end(), [StartsWithName](const std::string &Item) noexcept
                                   { return Item.rfind(StartsWithName, 0) == 0; });
        if (Result != Parts.end())
        {
    ...
    const std::string &GetFileName() const noexcept
    {
        ...
        auto Result = std::find_if(Parts.begin(), Parts.end(), [](const std::string &Item) noexcept
